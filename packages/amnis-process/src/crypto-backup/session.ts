import type { Session, CryptoEncoded } from '@amnis/core';
 ...
 * Encode a session.
 */
export function sessionEncode(session: Session, secret = processConfig.PROCESS_SESSION_SECRET) {
  const sessionPrep = {
    ...session,
...
export function sessionVerify(
  ...
  secret = processConfig.PROCESS_SESSION_SECRET,
): Session | undefined {
  try {
